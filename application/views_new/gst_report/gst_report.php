		...
		<tbody>
			<?php
				...
				foreach ($form_data['sale'] as $row) {
					...
					$str=substr($row['gstin'],0,2);
					if($str=="20")
					{
						echo "<td>".($row['total_tax']/2)."</td>";
						echo "<td>".($row['total_tax']/2)."</td>";
						$cgst+=($row['total_tax']/2);
						$sgst+=($row['total_tax']/2);
	...
	<table class="table table-striped table-border table-bordered" id="headerTable">
		...
		<tbody>
			<?php
				...
				//var_dump($form_data);
				//exit;
				foreach ($form_data['purchase'] as $row) {
					echo "<tr>";
					echo "<td>".$ctr."</td>";
					echo "<td>".$row['remarks']."</td>";
					echo "<td>".$row['party_name']."</td>";
					echo "<td>".$row['GSTIN']."</td>";
					echo "<td>".$row['taxable_value']."</td>";
					echo "<td>".$row['tax']."</td>";
					$str=substr($row['GSTIN'],0,2);
					$totaltax=($row['taxable_value']*$row['tax'])/100;
					if($str=="20")
					{
						echo "<td>".($totaltax/2)."</td>";
						echo "<td>".($totaltax/2)."</td>";
						$cgst+=($totaltax/2);
						$sgst+=($totaltax/2);
					...
					echo "<td>".($row['taxable_value']+$totaltax)."</td>";
					$ctr++;
				}
				?>
				<tr>
