        ...
        <div class="col-lg-12">
            <div class="ibox float-e-margins">
                <div class="ibox-title">
                    ...
                    <span class="pull-right"><a href="<?php echo site_url('rentstocktransfer') ?>"><i class="fa fa-angle-left"></i>Back</a> </span>

                </div>
                <?php } ?>
                <div class="ibox-content">
                    <form method="POST" class="form-horizontal" id="frm_rentstocktransfer">
                        ...
                        <div class="form-group  row"><label class="col-sm-2 control-label">Item</label>
                            <div class="col-sm-10">
                                <select class="form-control rec master_item_id" name="master_item_id">
                                    <option value="">ITEM</option>
                                    <?php
                                        foreach ($item_master as $row) {
                                            ?>
                                            <option value="<?=$row['master_item_id']?>"><?=$row['item_name']." -> ".$row['item_code']?></option>
                                            <?php
                                        }
                                    ?>
                                </select>
                        ...
                        <div class="form-group  row"><label class="col-sm-2 control-label">Warehouse</label>
                            <div class="col-sm-10">
                                <select class="form-control rec to_warehouse_id" name="to_warehouse_id">
                        ...
                        <div class="hr-line-dashed"></div>
                        <button type="button" class="btn btn-success" id="btn_save_rentstocktransfer">Save</button>
                    </form>
                </div>
                <?php
                if(!isset($condition)) {
...
<script type="text/javascript">
    ...
    $("#btn_save_rentstocktransfer").click(function () {
        var ref=$(this);
        if(validate(ref))
        {
            ...
            $.ajax({
                type: 'POST',
                url: '<?php echo site_url('asign_rent_item/add')?>',
                cache: false,
                async: false,
                data: $("#frm_rentstocktransfer").serialize(),
                success: function (data) {
                    //swal("","Record Saved Successfully","success");
                    lastid=data;
                    $("#frm_rentstocktransfer")[0].reset();
    ...
    $(".master_item_id").change(function(){
        ...
        if(item_id!="" && from_warehouse_id!="")
        {
           $.ajax({
                type: 'POST',
                url: '<?php echo site_url('asign_rent_item/getRentStockByItem')?>',
                cache: false,
                async: false,
