                ...
                <div class="ibox-content">
                    <form method="POST" class="form-horizontal" id="frm_vehicle_inward">
                        <div class="form-group  row"><label class="col-sm-2 control-label">vehicle No.</label>
                            <div class="col-sm-10"><input type="text" class="form-control rec vehicle_inward_no"  name="vehicle_inward_no"></div>
                        </div>
                        <div class="hr-line-dashed"></div>
                        <div class="form-group  row"><label class="col-sm-2 control-label">Driver Name</label>
                            <div class="col-sm-10"><input type="text" class="form-control"  name="driver_name"></div>
                        </div>
                        <div class="hr-line-dashed"></div>
                        <div class="form-group  row"><label class="col-sm-2 control-label">Mobile Number</label>
                            <div class="col-sm-10"><input type="number" class="form-control"  name="mobile_no"></div>
                        </div>
                        <div class="hr-line-dashed"></div>

                        <div class="form-group  row" style="display:none;"><label class="col-sm-2 control-label">License Number</label>
                            <div class="col-sm-10"><input type="number" class="form-control "  name="license_no"></div>
                        </div>
...
<script type="text/javascript">
    ...
    $("#btn_save_vehicle_inward").click(function () {
        ...
        if(validate(ref))
        {
            $.ajax({
                type: 'POST',
                url: '<?php echo site_url('vehicle_inward/add')?>',
                cache: false,
                async: false,
...
$(".vehicle_inward_no").change(function(){
    var vehicle_inward_no=$(this).val();
    $.ajax({
            type: 'POST',
            url: '<?php echo site_url('vehicle_inward/validation')?>',
            cache: false,
            async: false,
            data: {vehicle_inward_no:vehicle_inward_no},
            success: function (data) {
                if(data>0)
                    alert("vehicle_inward no already exist");
            },
        timeout: 5000,
        error: function(jqXHR, textStatus, errorThrown) {
            swal("","Please check your internet connection.","error");
