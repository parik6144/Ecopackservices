                            ...
                            <div class="col-sm-4">
                                <div class="form-group" id="data_1">
                                    ...
                                    <div class="input-group date">
                                        ...
                                        <input type="text" name="consignment_date" class="form-control purchase_date rec" value="<?=date("d-m-Y",strtotime($form_data['consignment']->consignment_date))?>">
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-sm-4">
                                <div class="form-group"><label>Consignment Type</label>
                                    <select class="rec form-control consignment_type" id="consignment_type" name="consignment_type">
                                        ...
                                        <option value="1" <?php if($form_data['consignment']->consignment_type=="1") echo "selected='selected'"; ?> style="display: none;">Rent</option>
                                    </select>
                                </div>
                            </div>

                            <div class="col-sm-4">
                                <div class="form-group"><label class="col-lg-8 col-form-label">Consignment No </label>
                                    <div class="col-lg-4"><input type="text" readonly placeholder="<?=$form_data['consignment']->consignment_no?>" class="form-control"></div>
                                </div>
                            ...
                            <div class="col-sm-12">
                                <div class="table-responsive">
                                    <table class="table table-stripped table-bordered">
                                        ...
                                        <tr class="rent_stock_tr">
                                            <td colspan="3">
                                                <table class="table table-stripped table-bordered">
                                                    ...
                                                    <?php if(isset($form_data['rent_stock_item'])) { foreach ($form_data['rent_stock_item'] as $row) { ?>
                                                    <tr class="rent_stock_tr">
                                                        <td>
                                                        ...
                                                        <td><input type="text" name="rent_stock_item_qty[]" class="form-control rent_stock_item_qty qty" placeholder="Qty" value="<?=$row['qty']?>" autocomplete="off"></td>
                                                        <td>
                                                            <button class="btn btn-info btn-circle add_new_rent_stock_item" type="button" title="Add new"><i class="fa fa-plus"></i></button>
                                                            <button class="btn btn-danger btn-circle btn_delete_rent_stock_item" type="button" title="Delete"  style="display: none;"><i class="fa fa-trash"></i>
                                                            </button>
                                                        </td>
                                                    </tr>
                                                    <?php }} ?>
                                        ...
                                        <?php foreach ($form_data['consignment_item'] as $row) { ?>
                                        <tr>
                                        ...
                                        <tr><td colspan="3"><h4 class="text-center">Transport Vehicle Item</h4></td></tr>

                                        <?php
                                        $consignment_id = $form_data['consignment']->consignment_id;
//                                        echo $sql="SELECT * FROM `tbl_consignment_transport_billing_stock_item` where consignment_id=$consignment_id";
//                                        $query=$this->db->query($sql);
//                                        $results=$this->$query->get();
//                                        foreach($results as $res):

                                        $query = $this->db->select('*')->from('tbl_consignment_transport_billing_stock_item')
                                        ->where(array("consignment_id"=>$consignment_id))->get();
                                        // echo $this->db->last_query(); exit;
                                        $consignment_transport_vehicle_item = $query->result_array();
                                        // print_r($consignment_transport_vehicle_item);
                                        foreach($consignment_transport_vehicle_item as $res):
                                        ?>
                                        ...
                                        <tr>
                                            <td>
                                                ...
                                                <option value=''>Select Transport Item</option>
                                                <?php foreach ($transport_vehicle_item as $row) { ?>
                                                    <?php if($res['item_id'] == $row['rent_item_id']) { ?>
                                                    <option value="<?= $row['rent_item_id'] ?>" selected><?=$row['rent_item_name'] ?></option>
                                                <?php }} ?>
                                                </select>
                                                <input type='hidden' name='transitemPrice' class='transitemPrice' value="<?=$res['item_id']?>">
                                                <input type="hidden" name="transitemadvPrice" class="transitemadvPrice" value="<?=$res['transitemadvPrice']?>">
                                                <input type="hidden" name="transitemduePrice" class="transitemduePrice" value="<?=$res['transitemduePrice']?>">
                                            </td>
                                            <td><input type="text" name="transqty[]" class="form-control transqty" placeholder="Qty" value="<?=$res['qty']?>" autocomplete="off" onchange="calculate_adv_due_price(this);"></td>
                                            <td>
                                        ...
                                        <?php foreach ($form_data['consignment_stock_item'] as $row) { ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <button class="btn btn-info btn-circle add_new" type="button" title="Add new" style="display: none;">
                                                    <i class="fa fa-plus"></i></button>
                                                    <button class="btn btn-danger btn-circle btn_delete" type="button" title="Delete">
                        ...
                        <div class="row">
                            <div class="col-sm-6">
                                ...
                                <div class="form-group"><label class="control-label ">Value</label>
                                    <div class=""><input type="text" class="form-control custom_control" value="<?=$form_data['consignment']->consignment_value?>" name="consignment_value"></div>
                                </div>
                                <div class="form-group"><label class=" control-label">D.C No.</label>
                                    <div class=""><input type="text" class="form-control custom_control"  value="<?=$form_data['consignment']->d_c_no?>"  name="d_c_no"></div>
                                </div>
                                <div class="form-group"><label class=" control-label">Party GST No.</label>
                                    <div class=""><input type="text" class="form-control custom_control consignee_gst_no"  name="party_gst_no" readonly="readonly" value="<?=$gstno?>"></div>
                                </div>
                            ...
                            <div class="col-sm-6">
                                <div class="form-group"><label class="control-label">Advance</label>
                                    <div class="">
                                        <input type="text" class="form-control rec custom_control advance"  name="advance" value="<?=$form_data['consignment']->advance ?>">
                                        <input type="hidden" class="hidden_advance"  name=""  value="<?=$form_data['consignment']->advance ?>" >
                                    </div>
                                </div>
                                <div class="form-group"><label class=" control-label">Due</label>
                                    <div class="">
                                        <input type="text" class="form-control rec custom_control due"  name="due"  value="<?=$form_data['consignment']->due ?>" >
                                        <input type="hidden" class="hidden_due"  name="" value="<?=$form_data['consignment']->due ?>">
                                ...
                                <div class="form-group"><label class=" control-label">Way Bill No.</label>
                                    <div class=""><input type="text" class="form-control custom_control"  name="way_bill_no" value="<?=$form_data['consignment']->way_bill_no ?>" ></div>
                                </div>
                                <div class="form-group"><label class="control-label">vehicle Owner</label>
                                    <div class=""><input type="text" readonly="readonly" class="form-control custom_control owner_name" value="<?=$form_data['consignment']->driver_name ?>" id="vehicle_owner"></div>
                                </div>
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if(rec.amount.vehicle_price.length>0)
                    {
                        ...
                        if(advance!="") { advance=parseInt(advance); }
                        if(due!=""){ due=parseInt(due); }
