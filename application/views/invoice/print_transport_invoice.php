...
<table class="tg" style="width:100%;">
  ...
  <?php 
    if(!empty($record['invoice']->order_no))
      echo "Purchase Order No:- ".$record['invoice']->order_no;
    ?>
...
foreach ($record['item'] as $row) {
    if (
        ($row['ref_table_name'] == "tbl_vehicle_inward") &&
        (($row['item_id'] == '10') || ($row['item_id'] == '11')) &&
        ($record['invoice']->billing_address_id == '2')
    ) {
        $tempqty += $row['qty'];
        $tempprice = $row['price'];
    } else {
?>
        <tr>
            ...
            <?php
                ...
                $itemName = $row['vehicle_type'];
            ?>
                <td class="tg-c3ow"><?= $row['vehicle_type'] ?></td>
            <?php
            }

            if ($record['invoice']->billing_address_id == '2' || $record['invoice']->billing_address_id == '24') {
            ?>
                <td class="tg-c3ow">996514</td>
            <?php
            } else {
            ?>
...

{
  ...
  <table class="tg" align="center">
    ...
    <tr>
      ...
      <th class="tg-us36" style="width:380px; overflow: hidden; display: inline-block;"><span >DC No.</span></th>
  ...
  <tbody>
    ...
    <?php
      ...
      foreach ($record['annexure']['inward'] as $row) {
        if($tblnm!="tbl_item" && $row['vehicle_id']=="152")
        {
        ...
        <tr>
          ...
          <td class="tg-dvpl" style="width:380px; overflow: hidden;"><?=$row['d_c_no']?></td>
...
<?php
  ...
  <?php if(sizeof($record['annexure']['outward'])>0) { ?>
  ...
  <table class="tg" align="center">
    <tr>
      ...
      <th class="tg-us36" style="width:380px; overflow: hidden; display: inline-block; border-bottom: none; ">DC No.</th>
  ...
  <tbody>
    ...
    <?php
      ...
      foreach ($record['annexure']['outward'] as $row) {
        ...
        <tr>
          <td class="tg-dvpl" ><?=$ctr?></td>
          <td class="tg-dvpl" style="width:380px; overflow: hidden; display: inline-block; border-bottom: none; "><?=$row['d_c_no']?></td>
          <td class="tg-dvpl" ><?=$row['consignment_no']?></td>
          <td class="tg-dvpl" ><?=date('d-m-Y',strtotime($row['consignment_date']))?></td>
          <?php
          if($tblnm=="tbl_item")
