                ...
                <div class="ibox-content">
                    <form method="POST" class="form-horizontal" id="frm_loan">
                        <div class="form-group  row"><label class="col-sm-3 control-label">Party Name</label>
                            <div class="col-sm-9">
                                <input type="hidden" value="<?php echo $this->input->get('id'); ?>" name="loan_id">
                                <select class="form-control rec" name="account_id">
                                    <option value=''>Select Party</option>
                                    <?php
                                    foreach ($account as $row) {
                                        $str='';
                                        if($form_data['loan']['0']['account_id']==$row['account_id'])
                                            $str="selected='selected'";
                                        echo "<option value='".$row['account_id']."' ".$str.">".$row['party_name']."</option>";
                                    }
                                    ?>
                                </select>
                            </div>
                        ...
                        <div class="form-group  row"><label class="col-sm-3 control-label">Loan Date</label>
                            <div class="col-sm-9"><input type="date" class="form-control rec" value="<?=$form_data['loan']['0']['loan_date']?>"  name="loan_date"></div>
                        </div>
                        <div class="hr-line-dashed"></div>
                        <div class="form-group  row"><label class="col-sm-3 control-label">Type</label>
                            <div class="col-sm-9">
                                <select class="form-control loan_type rec" name="loan_type">
                                    <option value="">Select Loan Type</option>
                                    <option value="1" <?php if($form_data['loan']['0']['loan_type']=="1") echo "selected='selected'";?>>EMI</option>
                                    <option value="2" <?php if($form_data['loan']['0']['loan_type']=="2") echo "selected='selected'";?>>Interest</option>
                                </select>
                            </div>
                        </div>
                        <div class="hr-line-dashed"></div>
                        <div class="form-group  row"><label class="col-sm-3 control-label">Loan Amount</label>
                            <div class="col-sm-9"><input type="number" class="form-control rec loan_amount" value="<?=$form_data['loan']['0']['loan_amount']?>"  name="loan_amount"></div>
                        </div>
                        <div class="hr-line-dashed"></div>
                        <div class="form-group  row due_amount_div"><label class="col-sm-3 control-label">Due Amount</label>
                            <div class="col-sm-9"><input type="number" class="form-control due_amount" value="<?=$form_data['loan']['0']['due_amount']?>" name="due_amount"></div>
                        </div>
                        <div class="hr-line-dashed"></div>
                         <div class="form-group  row"><label class="col-sm-3 control-label">Interest Rate</label>
                            <div class="col-sm-9"><input type="number" class="form-control interest_rate rec" value="<?=$form_data['loan']['0']['interest_rate']?>" name="interest_rate"></div>
                        </div>
                        <div class="hr-line-dashed"></div>
                         
                       <div class="form-group  row time_div"><label class="col-sm-3 control-label">Tennure in months</label>
                            <div class="col-sm-9"><input type="number" class="form-control rec time" value="<?=$form_data['loan']['0']['loan_time']?>" name="loan_time"></div>
                        </div>
                        <div class="hr-line-dashed"></div>

                        <div class="form-group  row"><label class="col-sm-3 control-label">Monthly EMI</label>
                            <div class="col-sm-9"><input type="number" class="form-control rec monthly_emi" value="<?=$form_data['loan']['0']['monthly_emi']?>" name="monthly_emi"></div>
                        </div>
                        <div class="hr-line-dashed"></div>
...
<script type="text/javascript">
    ...
    $("#btn_save_loan").click(function () {
        ...
        if(validate(ref))
        {
            $.ajax({
                type: 'POST',
                url: '<?php echo site_url('loan/edit')?>',
                cache: false,
                async: false,
