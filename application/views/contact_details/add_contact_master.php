            ...
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content">
                    <form method="POST" class="form-horizontal" id="frm_contact">
                        <div class="form-group  row"><label class="col-sm-3 control-label">Company Name</label>
                            <div class="col-sm-9">
                                <input type='hidden' name="contact_type" id="contact_type">
                                <select class="form-control consignee_name rec" name="consignee_id">
                                    <option value="">Select Company</option>
                                <?php
                                foreach ($consignee as $row) {
                                    ?>
                                    <option value="<?=$row['consignee_id']?>" type="0"><?=$row['consignee_name']?></option>
                                    <?php
                                }
                                ?>
                                <?php
                                foreach ($consignor as $row) {
                                    ?>
                                    <option value="<?=$row['consignor_id']?>" type="1"><?=$row['consignor_name']?></option>
                                    <?php
                                }
                                ?>
                                </select>
                        ...
                        <div class="form-group  row"><label class="col-sm-3 control-label">Purpose</label>
                            <div class="col-sm-9"><input type="text" class="form-control "  name="reason"></div>
                        </div>
                        ...
                        <div class="form-group  row"><label class="col-sm-3 control-label">Email ID</label>
                            <div class="col-sm-9"><input type="text" class="form-control  rec"  name="email_id"></div>
                        </div>
                        <div class="hr-line-dashed"></div>
                        <div class="form-group  row"><label class="col-sm-3 control-label">Person Name</label>
                            <div class="col-sm-9"><input type="text" class="form-control rec"  name="person_name"></div>
                        </div>                        
                        <div class="hr-line-dashed"></div>
   ...
   $(".consignee_name").change(function(){
    ...
    $("#btn_save_contact").click(function () {
        ...
        if(validate(ref))
        {
            $.ajax({
                type: 'POST',
                url: '<?php echo site_url('contactDetails/add')?>',
                cache: false,
                async: false,
                ...
                success: function (data) {
                    ...
                    $("#frm_contact")[0].reset();
                    $('#_contactDetailsModal').modal('hide');
                },
                error: function (data) {
                    // alert("error");
                },
