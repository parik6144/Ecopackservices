		...
		<?php
			...
			foreach ($form_data['inward'] as $row) {
				if($date!=$row['date'])
				{
					...
					$previouscno=$row['vehicle_inward_no'];
				}
				elseif (!empty($row['vehicle_inward_no'])&& $row['vehicle_inward_no']!=$previouscno) {
					if(isset($tempin))
					{
						array_push($itemin, $tempin);
						array_push($itemout, $tempout);
		...
		<tbody>
			...
			for($i=0;$i<sizeof($dateary);$i++)
			{
				if($prevdate!=$dateary[$i])
				{
					...
					echo "</tr>";
					$prevdate=$dateary[$i];
					$prevlrno=$vehiclenoary[$i];
					$ctr++;
				}
				else if($prevlrno!=$vehiclenoary[$i] && !empty($vehiclenoary[$i]))
				{
					echo "<tr>";
					echo "<td>".date("d-m-Y",strtotime($dateary[$i]))."</td>";
					for($j=0;$j<sizeof($opstock);$j++)
