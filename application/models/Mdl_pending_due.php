<?php
class Mdl_pending_due extends CI_Model{
    ...
    public function changestatus($consignment_id)
    {
        ...
        $ary=array('consignment_id' => $consignment_id,"consignee_id"=>$data->consignee_id,"amount"=> $data->due,"owner_id"=>$data->owner_id,"created_by"=>$user_id,"created_datetime"=>$datetime,"payment_mode"=>$payment_mode);
        $this->db->insert('tbl_payment_due',$ary);
        
    }
    public function getconsignmentbyid($consignmentid)
    {
        $query=$this->db->select('consignment_id,tbl_consignment.consignee_id, due,tbl_inward_owner.owner_id')
    ...
    //     }*/
    //     $col = (int)$column;
    //     $arr=array("consignment_no", "consignment_date", "vehicle_inward_no","owner_name","tbl_inward_owner.mobile_no", "bank_name", "branch","account_no","ifsc_code","due");
    //     $this->db->select('tbl_consignment.consignment_id,consignment_no, consignment_date, vehicle_inward_no,owner_name,tbl_inward_owner.mobile_no,bank_name,branch,account_no,ifsc_code,tbl_outward_rate.due,tbl_consignment.bill_type,sum(tbl_consignment_stock_item.qty) as qty');
    //     if(!empty($searchstr))
    //     {
    ...
    //         $this->db->or_like('tbl_outward_rate.due', $searchstr);
    //     }
        
    //         $this->db->where('tbl_consignment.consignment_id NOT IN (select consignment_id from tbl_payment_due)', NULL, FALSE);
    //         $this->db->where('tbl_outward_rate.due >','0');
    //         $this->db->where('tbl_outward_rate.payment_mode','0');
    //         $this->db->where('tbl_consignment.consignment_status','1');
      ...
      public function getconsignment($start="",$length="",$searchstr="",$column,$type)
    {
        ...
        $col = (int)$column;
        $arr=array("consignment_no", "consignment_date", "vehicle_inward_no","owner_name","tbl_inward_owner.mobile_no", "bank_name", "branch","account_no","ifsc_code","due");
        $this->db->select('tbl_consignment.consignment_id,consignment_no, consignment_date, vehicle_inward_no,tbl_inward_owner.owner_name,tbl_inward_owner.mobile_no,bank_name,branch,account_no,ifsc_code,tbl_consignment.due,tbl_consignment.bill_type,sum(tbl_consignment_stock_item.qty) as qty');
        if(!empty($searchstr))
        {
            $this->db->or_like('consignment_no', $searchstr);
            $this->db->or_like('consignment_date', $searchstr);
            ...
            $this->db->or_like('account_no', $searchstr);
            $this->db->or_like('ifsc_code', $searchstr);
            $this->db->or_like('tbl_consignment.due', $searchstr);
        }
        
            $this->db->where('tbl_consignment.consignment_id NOT IN (select consignment_id from tbl_payment_due)', NULL, FALSE);
            $this->db->where('tbl_consignment.due >','0');
            $this->db->where('tbl_outward_rate.payment_mode','0');
            $this->db->where('tbl_consignment.consignment_status','1');
        $tempdb = clone $this->db;
        $this->db->order_by($arr[$col],$type);
    ...
    public function gettotaldue()
    {
        ...
        $this->db->select('tbl_consignment.consignment_id,tbl_outward_rate.due,tbl_consignment.bill_type,sum(tbl_consignment_stock_item.qty) as qty');
        $this->db->from('tbl_consignment');
        if(sizeof($ids)>0)
