...
class Mdl_invoice extends CI_Model
{
    ...
    {
        parent::__construct();
        $this->invoiceprefix = $this->getInvoicePrefix(); // Constructor me initialize karein
    }
    ...
    //     } else {
    //         $financialYear = ($currentYear - 1) . '-' . date('y');
    //     }

    //     return "ESPL/{$financialYear}/";
    // }
    
    public function getInvoicePrefix()
    {
        $currentYear = date('y'); // Last 2 digits
        $nextYear = date('y', strtotime('+1 year'));
    
        if (date('m') >= 4) {
            $financialYear = $currentYear . '-' . $nextYear;
        } else {
            $financialYear = date('y', strtotime('-1 year')) . '-' . $currentYear;
        }
