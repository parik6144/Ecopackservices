...
class Mdl_staff extends CI_Model{
    ...
    public function savestaff($post,$imagename)
    {
        ...
        if(empty($_POST['location'])) $location=""; else $location=$_POST['location'];
    ...
    public function getstaff($start="",$length="",$searchstr="",$column,$type)
    {
        ...
        {
            $this->db->select('`staff_id`,`location`, `emp_no`, type_name,`staff_name`, `mobile_no`, `email_id`, `gstin`, `bank_name`, `branch`, `account_no`, `ifsc_code`,photo,blood_group');
            if(!empty($searchstr))
            {
