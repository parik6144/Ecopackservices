...
class Mdl_warehouse extends CI_Model{
    ...
    public function savewarehouse($post)
    {
        ...
        if(empty($_POST['warehouse_name'])) $warehouse_name=""; else $warehouse_name=$_POST['warehouse_name'];
        if(empty($_POST['warehouse_city'])) $warehouse_city=""; else $warehouse_city=$_POST['warehouse_city'];
        if(empty($_POST['name_warehouse'])) $name_warehouse=""; else $name_warehouse=$_POST['name_warehouse'];
        ...
        $data=array("warehouse_name"=>$warehouse_name,"name_warehouse"=>$name_warehouse,"warehouse_city"=>$warehouse_city,"place_id"=>$warehouse_city, "warehouse_address"=>$warehouse_address,"warehouse_gst_no"=>$warehouse_gst_no,"created_by"=>$user_id,"created_datetime"=>$datetime);
    ...
    public function getwarehouse($start="",$length="",$searchstr="",$column,$type)
    {
        ...
        {
            $query=$this->db->select('warehouse_id,`place_name`,`name_warehouse`,`warehouse_city`,`warehouse_name`,warehouse_address,warehouse_gst_no')
                ->from('tbl_warehouse')
                ->join('place','place.place_id=tbl_warehouse.place_id')
        ...
        {
            $this->db->select('warehouse_id,`place_name`,`name_warehouse`,`warehouse_city`,`warehouse_name`,warehouse_address,warehouse_gst_no');
            $this->db->where('tbl_warehouse.is_deleted', '0');
            if(!empty($searchstr))
            {
            ...
            $this->db->join('place','place.place_id=tbl_warehouse.place_id');
    ...
    {
        $query=$this->db->select('`warehouse_name`,place_id,warehouse_id,warehouse_address,warehouse_gst_no')
            ->from('tbl_warehouse')
    ...
    {
        $query=$this->db->select('place_id,place_code,`place_name`')
            ->from('place')
            ->where('is_deleted', '0')
            ->get();
        return $query->result_array();
    ...
    public function updatewarehouse($post)
    {
        ...
        $user_id=$this->session->userdata('user_id');
        $datetime=date('Y-m-d h:i:s');
        $data=array("warehouse_name"=>$warehouse_name,"warehouse_address"=>$warehouse_address,"place_id"=>$warehouse_city,"warehouse_gst_no"=>$warehouse_gst_no,"updated_datetime"=>$datetime,"updated_by"=>$user_id);
