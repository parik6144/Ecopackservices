    ...
    public function saveconsignment($post)
     {
         ...
         if(empty($_POST['bill_type']))
             $bill_type=""; else $bill_type=$_POST['bill_type'];

         if(empty($_POST['consignment_value'])) $consignment_value="";
         else $consignment_value=$_POST['consignment_value'];
...
//       else $consignment_type=$_POST['consignment_type'];
       ...
       $barcodes=(json_decode($_POST['itemsbarcode'],true));
        ...
        {
            if(!empty($_POST['rent_stock_item_id'][$i]))
            {
                // echo sizeof($_POST['item_barcode']); exit();
                for($j=0; $j<sizeof($barcodes[$_POST['rent_stock_item_id'][$i]]); $j++)
                {
                // CHECKING AVAILABILITY STARTS HERE.
                $dataBarcode = array("item_id" => $_POST['rent_stock_item_id'][$i], "barcode_no" => $barcodes[$_POST['rent_stock_item_id'][$i]][$j], "dc_no" => $dc_no,"CreateDatetime"=>date('Y-m-d H:i:s'));
                $this->db->insert('tbl_item_outward_barcodes', $dataBarcode);
                //echo $this->db->last_query(); exit();
        ...
        // Inserting Barcodes ends here.
         ...
         if(sizeof($rentdata)>0) $this->db->insert_batch('tbl_consignment_rent_stock_item', $rentdata);
        return true;
     }
...
//          $this->db->where(array("tbl_consignment.is_dc"=>1) OR array("tbl_consignment.consignment_no"=>$searchstr));
    ...
    function getconsignmentforprint($consignmentid)
    {
       ...
       $query=$this->db->select('`tbl_consignment_rent_stock_item`.`item_id`,`tbl_rent_item_master`.`item_name`,`tbl_consignment_rent_stock_item`.`qty`')
         ...
         ->join('tbl_rent_item_master','`tbl_rent_item_master`.`master_item_id`=`tbl_consignment_rent_stock_item`.`item_id`', 'left')
