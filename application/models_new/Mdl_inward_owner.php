 ...
 * Time: 11:49
 */
class Mdl_inward_Owner extends CI_Model{

    public function saveinward_owner ($post)
    {
        ...
        $this->db->insert('tbl_inward_owner',$data);
        $lastid=$this->db->insert_id();        
        return $lastid;


    }
    public function getinward_owner($start="",$length="",$searchstr="",$column,$type)
    {
        $col = (int)$column;
        if($start=='' && $length=='' && $searchstr=='')
        {
            $query=$this->db->select('`owner_id`, `owner_name`, `mobile_no`')
        ...
        else
        {
            ...
            // }
             if(!empty($searchstr))
            {
                $where  = "(`owner_name` LIKE '%$searchstr%' OR ";
                $where .= "`mobile_no` LIKE '%$searchstr%' OR";
                $where .= "`city` LIKE '%$searchstr%' OR";
                $where .= "`pincode` LIKE '%$searchstr%' OR";
                $where .= "`gstin` LIKE '%$searchstr%' OR";
                $where .= "`bank_name` LIKE '%$searchstr%' OR";
                $where .= "`branch` LIKE '%$searchstr%' OR";
                $where .= "`account_no` LIKE '%$searchstr%' OR";
                $where .= "`ifsc_code` LIKE '%$searchstr%')";
                $this->db->where($where);
            }
            ...
            $num_rows = $tempdb->from('tbl_inward_owner')->count_all_results();
            if($length>0)
            $this->db->limit($length, $start);
            $this->db->from('tbl_inward_owner');
            // $this->db->where('is_deleted', 0);
    ...
    public function getinward_ownerbyid($ownerid)
    {
    ...
    public function updateinward_owner($post)
    {
        if(empty($_POST['owner_name']))
            $owner_name="";
        else
        ...
        $this->db->update('tbl_inward_owner',$data);
        return $lastid;
    }
