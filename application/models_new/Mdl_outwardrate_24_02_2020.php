...
class Mdl_outwardrate extends CI_Model{
    ...
    public function saveoutwardrate($post)
    {
        ...
        $data=array("consignee_id"=>$consignee_id,"consignor_id"=>$consignor_id,"vehicle_type_id"=>$vehicle_type_id,"advance"=>$advance,"driver_price"=>$driver_price,"due"=>$due,"employee_charge"=>$employee_charge,"bill_type"=>$bill_type,"payment_mode"=>$payment_mode,"created_by"=>$user_id,"created_datetime"=>$datetime);
    ...
    public function getoutwardrate($start="",$length="",$searchstr="",$column,$type)
    {
        ...
        if($start=='' && $length=='' && $searchstr=='')
        {
            ...
            $query=$this->db->select('`rate_id`, `consignee_id`, `vehicle_type_id`, `driver_price`, `advance`, `due`, `employee_charge`, `bill_type`,payment_mode')
                ->from('tbl_outward')
                ->get();
        ...
        {
            $this->db->select('`rate_id`, `consignor_name`,`consignee_name`, `vehicle_type`, `driver_price`, `advance`, `due`, `employee_charge`, `bill_type`,payment_mode');
            if(!empty($searchstr))
            {
