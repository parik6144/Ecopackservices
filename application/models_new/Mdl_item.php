...
class Mdl_item extends CI_Model{
    ...
    public function saveitem ($post)
    {
        ...
        if(empty($_POST['item_name'])) $item_name=""; else $item_name=$_POST['item_name'];

        if(empty($_POST['item_code'])) $item_code=""; else $item_code= $_POST['item_code'];
        ...
        if($item_type=="transport"){
            $data=array("item_name"=>$item_name,"item_code"=>$item_code,"consignee_id"=>$consignee_id,"opening_stock"=>$opening_stock,"created_by"=>$user_id,"created_datetime"=>$datetime);
    ...
    public function getitem($start="",$length="",$searchstr="",$column,$type)
    {
        ...
        else
        {
            ...
            $this->db->select('`item_id`, `item_name`,`item_code`,consignee_name,opening_stock');
            if(!empty($searchstr))
            {
    ...
    public function getrentitem($start="",$length="",$searchstr="",$column,$type)
    {
        ...
        else
        {
            ...
            $this->db->select('`item_id`,`item_code`,`item_name`,consignee_name,tbl_stock_rent_item.opening_stock,tbl_rent_item_master.master_item_id,tbl_rent_item_master.price,tbl_stock_rent_item.consignee_id');
            if(!empty($searchstr))
            {
                $this->db->or_like('item_name', $searchstr);
                $this->db->or_like('consignee_name', $searchstr);
    ...
    {
        $query=$this->db->select('`tbl_stock_rent_item`.`master_item_id`, `item_name`')
            ->from('tbl_stock_rent_item')
            ->join('tbl_rent_item_master', 'tbl_rent_item_master.master_item_id = tbl_stock_rent_item.master_item_id', 'left')
            ->where(array("consignee_id"=>$consigneeid))
            ->get();
            // echo $this->db->last_query();
            $CountRentItems = $query->num_rows();
            if($CountRentItems>0)
            {
                return $query->result_array();
            }
            else
            {
                ...
                echo 'alert("No items added in records for this consignee.")';
                echo '</script>';
