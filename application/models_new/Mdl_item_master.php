...
class Mdl_item_master extends CI_Model{
    ...
    public function saveitem ($post)
    {
        ...
        $data=array("item_name"=>$item_name,"item_code"=>$item_code,"price"=>$price,"tax"=>$tax,"created_by"=>$user_id,"created_datetime"=>$datetime);
    ...
    public function getitem($start="",$length="",$searchstr="",$column,$type)
    {
        ...
        {
            $query=$this->db->select('`master_item_id`,`tax`,`item_name`, `item_code`, `price`')
        ...
        else
        {
            ...
            $this->db->select('`master_item_id`, `item_name`, `item_code`, `price`, `tax`');
            if(!empty($searchstr))
            {
    ...
    public function getbarcodesforitems($itemid)
    {
        $sql=$this->db->select('*')
            ->from('tbl_item_barcodes')
            ->where('tbl_item_barcodes.item_id', $itemid)
            ->get();
        return $sql->result_array();
    }
    ...
    {
        $query=$this->db->select('`master_item_id`, `item_name`, `item_code`,`tax`, `price`,tbl_warehouse_opening_stock.qty,tbl_warehouse_opening_stock.warehouse_id')
            ->from('tbl_rent_item_master')
    ...
    public function updateitem($post)
    {
       ...
       if(empty($_POST['opening_stock']))
        ...
        $datetime=date('Y-m-d h:i:s');
        $data=array("item_name"=>$item_name,"item_code"=>$item_code,"price"=>$price,"tax"=>$tax,"updated_by"=>$user_id,"updated_datetime"=>$datetime);
