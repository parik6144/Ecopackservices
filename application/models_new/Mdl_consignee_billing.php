    ...
    public function getconsignee_billing($start="",$length="",$searchstr="",$column,$type)
    {
        ...
        else
        {
            ...
            if(!empty($searchstr))
            {
                ...
                $where .= "`address` LIKE '%$searchstr%' OR";
                $where .= "`city` LIKE '%$searchstr%' OR";
                $where .= "`state` LIKE '%$searchstr%' OR";
                $where .= "`pincode` LIKE '%$searchstr%' OR";
                $where .= "`phone_no` LIKE '%$searchstr%' OR";
                $where .= "`mobile_no1` LIKE '%$searchstr%' OR";
                $where .= "`gstin` LIKE '%$searchstr%' OR";
                $where .= "`mobile_no` LIKE '%$searchstr%')";
                $this->db->where($where);
            }
            //echo 8888888; exit();
